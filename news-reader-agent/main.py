...
from tool import search_tool, scrape_tool
...
class NewsReaderAgent:
    ...
    @task
    def summary_translation_task(self):
        return Task(
            config=self.tasks_config["summary_translation_task"],
        )

    @task
    def final_report_translation_task(self):
        return Task(
            config=self.tasks_config["final_report_translation_task"],
        )
